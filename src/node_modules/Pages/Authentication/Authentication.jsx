...
const Authentication = () => {
    ...
    const getCookie = () => {
        axios.get(`https://sergei-sorokin.ru//api/auth/generate_auth_cookie/?username=${username}&password=${password}`)
            .then(res => {
                const data = res.data;
                if(data.status === 'ok'){
                    document.cookie = `${data.cookie_name}=${data.cookie}`;
                    setError('')
                }
    ...
    return (
        <div className={`${styles.authenticationWrapper} ${currentThemeState.isLight ? styles.light : styles.dark}`}>
                ...
                <form className={styles.form} onSubmit={handleSubmit}>
                    ...
                    <div className={styles.checkboxWrapper}>
                        <label htmlFor="remember" className={styles.labelCheckbox}>
                            <input type="checkbox" className={styles.checkbox} id='remember'/> Remember me
                    ...
                    <div className={styles.buttonWrapper}>
                        <ButtonFreeez text='LOG IN' theme='light' width='300px' align='center' type='submit'/>
                    </div>
