...
const TopBar = () => {
    ...
    const [currentShoppingBagState] = useContext(CurrentShoppingBagContext);
    ...
    const services = currentThemeState.isLang === 'ru' ? 'УСЛУГИ' : 'SERVICES';
    const training = currentThemeState.isLang === 'ru' ? 'ОБУЧЕНИЕ' : 'TRAINING';
    const contacts = currentThemeState.isLang === 'ru' ? 'КОНТАКТЫ' : 'CONTACTS';
    const team = currentThemeState.isLang === 'ru' ? 'КОМАНДА' : 'TEAM';
    const freeez = currentThemeState.isLang === 'ru' ? 'О “FREEEZ”' : 'about "FREEEZ"';
    const express_design = currentThemeState.isLang === 'ru' ? 'ЭКСПРЕСС ДИЗАЙН' : 'EXPRESS DESIGN';
    const game_design = currentThemeState.isLang === 'ru' ? 'ИГРОВОЙ ДИЗАЙН' : 'GAME DESIGN';

    useEffect(()=>{
        const {trainingBag, expressBag} = currentShoppingBagState;
        setBagLength(trainingBag.length + expressBag.length)
        dispatch({type: 'SET_HEIGHT_MENU', payload: heightMenuRef.current.clientHeight});
    }, [heightMenuRef, dispatch, currentShoppingBagState]);
        ...
        <div className={`${styles.topBarWrapper} ${currentThemeState.isLight ? styles.light : styles.dark}`} ref={heightMenuRef}>
            <nav className={styles.topBar}>
                ...
                <ul className={styles.topBarNav}>
                    ...
                    <li className={styles.topBarNavItem}>
                        <NavLink to='/training' className={styles.topBarNavLink}
                                 activeClassName={styles.topBarNavLinkActive}>
                            {training}
                        </NavLink>
                    </li>
                ...
                <ul className={styles.topBarIconNav}>
                    ...
                    <li className={styles.topBarIconNavItem}>
                        <NavLink to='/shopping_bag' className={styles.topBarIconNavLink}
                                 activeClassName={styles.topBarIconNavLinkActive}>
